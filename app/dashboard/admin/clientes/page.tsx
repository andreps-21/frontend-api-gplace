...
export default function AdminClientesPage() {
  ...
  const load = useCallback(async () => {
    setLoading(true)
    try {
      const raw = await apiService.getAdminCustomers({ page, per_page: 15, search: search || undefined })
  ...
  const remove = async (row: Record<string, unknown>) => {
    if (!confirm(`Eliminar cliente #${row.id}?`)) return
    try {
      await apiService.deleteAdminCustomer(Number(row.id))
    ...
    <div className="space-y-6">
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-lg">
          <DialogHeader>
            <DialogTitle>{editingId ? `Editar cliente #${editingId}` : "Novo cliente"}</DialogTitle>
            <DialogDescription>Campos alinhados à validação da API admin.</DialogDescription>
          </DialogHeader>
